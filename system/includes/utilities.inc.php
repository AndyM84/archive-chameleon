<?php
	...
	/*
	 * $Id: utilities.inc.php 116 2010-10-17 03:49:44Z amale $
	 */
	...
	function S($key, array $replacements = null) {
		...
		if (!isset($strings[$n2f->cfg->sys_lang])) {
			if ($n2f->debug->showLevel(N2F_DEBUG_ERROR)) {
				$n2f->debug->throwError(N2F_ERROR_NO_LANGUAGE_SET, S('N2F_ERROR_NO_LANGUAGE_SET'), 'system/includes/utilities.inc.php');
			}
		...
		if (!isset($strings[$n2f->cfg->sys_lang][$key])) {
			if ($n2f->debug->showLevel(N2F_DEBUG_WARN)) {
				$n2f->debug->throwWarning(N2F_WARN_LANG_KEY_MISSING, S('N2F_WARN_LANG_KEY_MISSING', array($key)), 'system/includes/utilities.inc.php');
			}
