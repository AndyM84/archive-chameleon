<?php
	...
	/*
	 * $Id: template.cls.php 99 2010-07-07 05:23:05Z amale $
	 */
	...
	define('N2F_TEMPLATE_DIR', N2F_REL_PATH . 'system/extensions/n2f_template/');

	require(N2F_TEMPLATE_DIR . 'config.inc.php');
	...
	if (is_array($cfg['tpl']['exts']) && count($cfg['tpl']['exts']) > 0) {
		foreach (array_values($cfg['tpl']['exts']) as $ext) {
			n2f_cls::getInstance()->loadExtension("n2f_template/{$ext}");
		}
	}

	// Setup our 'global' event
	n2f_cls::getInstance()->createEvent(N2F_TPLEVT_CREATED);
	...
	class n2f_template extends n2f_events {
		...
		public function __construct() {
			...
			$n2f->touchEvent(N2F_TPLEVT_CREATED, array(&$this));
